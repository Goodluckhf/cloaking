...
$(function() {
    ...
    initializeClock('land-time', dayEndTimestamp);
  ...
  $(".buy").click(function() {
      var product = $(this).attr('data-header');
      var price = $(this).attr('data-price');
      var id = $(this).attr('data-id');
      var currency = $(this).attr('data-currency');
      var basketJSON = {};

      basketJSON['item_id']  = id;
      basketJSON['price']  = price;
      basketJSON['currency']  = currency;
      basketJSON['quantity']  = 1;
    $('[name=address]').val(product);
    $('[name="dop_params[basket]"]').val(JSON.stringify(basketJSON));
      $(".orderform").animate({
          left: 0
      });
      return false;
...
function lastpack(numpack) {
    var minNumPack = 5; // Минимальное количество упаковок
